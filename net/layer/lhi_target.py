import copy
from net.layer.lhi_nms import lhi_encode
import random
import numpy as np
import torch
...
def make_one_lhi_target(cfg, input, proposal, truth_box, truth_label):
    ...
    if len(truth_box) == 0:
        num_bg = min(len(proposal), cfg['lhi_train_batch_size'])
    ...
    fg_index = np.where(max_overlap >= cfg['lhi_train_fg_thresh_low'])[0]
    bg_index = np.where(max_overlap <  cfg['lhi_train_bg_thresh_high'])[0]

    # sampling for class balance
    num_class = cfg['num_class']
    num = cfg['lhi_train_batch_size']
    num_fg = int(np.round(cfg['lhi_train_fg_fraction'] * cfg['lhi_train_batch_size']))
    ...
    if num_fg>0:
        ...
        sampled_target = lhi_encode(target_box, target_truth_box, cfg['box_reg_weight'])
...
def make_lhi_target(cfg, mode, inputs, proposals, truth_boxes, truth_labels, truth_masks):
    ...
    for b in range(batch_size):
        ...
        sampled_proposal, sampled_label, sampled_assign, sampled_target = \
           make_one_lhi_target(cfg, input, proposal, truth_box, truth_label)
