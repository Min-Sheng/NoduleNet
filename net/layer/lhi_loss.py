...
def lhi_loss(logits, deltas, labels, targets, deltas_sigma=1.0):
    ...
    lhi_cls_loss = F.cross_entropy(logits, labels, weight=weight, size_average=True)
    ...
    if num_pos > 0:
        ...
        lhi_reg_loss = Variable(torch.cuda.FloatTensor(1).zero_()).sum()

    return lhi_cls_loss, lhi_reg_loss, [reg_losses[0], reg_losses[1], reg_losses[2],
